...
import os
from mosaic_core.engine.models import classifiers, deeplab
...
MODEL_DICT = {
    ...
    'resnet8_t': classifiers.resnet8t.ResNet8
...
def get_dataset(name: str, data_root: str = 'data', return_transform=False, split=['A', 'B', 'C', 'D']):
    ...
    if name == 'cifar10':
        ...
        train_dst = datasets.ImageFolder(os.path.join(
            data_root, 'train'), transform=train_transform)
        val_dst = datasets.ImageFolder(os.path.join(
