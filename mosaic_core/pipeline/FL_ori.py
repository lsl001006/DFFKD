...
class OneTeacher:
    ...
    def update_round(self, roundd, selectN):
        ...
        for iter in tqdm(range(self.iters_per_round)):
            # 1. update D,G
            z = torch.randn(
                size=(self.args.batch_size, self.args.z_dim)).cuda()
    ...
    def update_netG_batch(self, syn_img, selectN):
        ...
        with self.args.autocast():
            ...
            if self.args.use_l1_loss:
                # (bs, ncls) L1 loss
                loss_adv = - \
                    self.criterion_distill(logits_S, ensemble_logits_T)
        ...
        for _ in range(5):
            with self.args.autocast():
                ...
                if self.args.use_l1_loss:
                    ...
                    loss = engine.criterions.kldiv(
                        loigts_S, logits_T.detach(), T=self.args.T)
