...
class OneTeacher:
    ...
    def init_training(self):
        ...
        self.iters_per_round = min(
            [len(local_loader.dataloader) for local_loader in self.local_dataloader])  # 4~9
        print(f'===iters_per_round:{self.iters_per_round}')
        ...
        #     self.optim_s, steps_all)  # TODO, eta_min=config.STU_LR_MIN,
        # for gen
    ...
    def update_round(self, roundd, selectN):
        ...
        for iter in tqdm(range(self.iters_per_round)):
            ...
            #         **registry.NORMALIZE_DICT[self.args.dataset]),
            # ])
            # img = utils.save_syn_img(syn_img[0].clone(), train_trans)
            # img.save('./1.png')
            # 归一化生成图像
...
class MultiTeacher(OneTeacher):
    ...
    def gen_dataset(self, args):
        # 通过dirichlet分布分割数据，为每个local提供由N_class类别组成的数量不一的数据
        # priv_train_data: 被分为N_PARTIES份数据，每份数据有N_class, 服从dirichlet分布
