...
def build_dataloader(args):
    ...
    if args.N_PARTIES==1:
        ...
        num_classes, tr_dataset, test_dataset = registry.get_dataset(name= args.dataset, data_root=args.DATAPATH) 
    ...
    test_loader = DataLoader(
        dataset=test_dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.workers, sampler=None)
    # local 
    local_datanum = np.zeros(args.N_PARTIES)
    local_cls_datanum = np.zeros((args.N_PARTIES, args.N_class))
    for localid in range(args.N_PARTIES):
        #count
