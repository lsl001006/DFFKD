...
def build_distillloader(args, publictype, online=True):
    # public cifar100
    if publictype == 'cifar100':
        public_dataset = Cifar_Dataset(
            os.path.join(args.DATAPATH, 'cifar-100-python/'),
            publictype,
            train=True,
            verbose=False,
            distill=True,
...
def dirichlet_datasplit(args, privtype='cifar10'):
    datapath = os.path.expanduser(args.DATAPATH)  # data/fed
    ...
    if privtype == 'cifar10':
        ...
        N_class = 100
    # 固定数据
    splitname = f'{args.DATAPATH}/splitfile/{privtype}/{args.alpha}_{args.seed}.npy'
    if os.path.exists(splitname):
        # split_arr包含了N_class类数据在N_parties个local上是按照何种概率分布的
        # sum(split_arr[i]) == 1.0
        split_arr = np.load(splitname)
        assert split_arr.shape == (N_class, N_parties)
    else:
        # dirichlet分布D(alpha, size), alpha越大，越接近于均匀分布，越小分布越集中
        ...
        if data_type == 'cifar10':
            if train == True:
                ...
                for i in range(1, 6):
                    ...
                    # 一个data_batch包含10000张图片，由RGB3通道构成, 32*32大小
